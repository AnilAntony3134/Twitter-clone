...
export const editUser = (id, formData, history) => async (dispatch, getState) => {
  ...
  try {
    ...
    const response = await axios.put(`http://localhost:80/api/users/${id}`, formData, options);
...
export const getProfile = (username, history) => async (dispatch, getState) => {
  ...
  try {
    const options = attachTokenToHeaders(getState);
    const response = await axios.get(`http://localhost:80/api/users/${username}`, options);
...
export const deleteUser = (id, history) => async (dispatch, getState) => {
  ...
  try {
    const options = attachTokenToHeaders(getState);
    const response = await axios.delete(`http://localhost:80/api/users/${id}`, options);
